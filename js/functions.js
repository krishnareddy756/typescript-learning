...
function getMobileByManufacturer(manufacturer = "Samsung", id) {
    let MobileList;
    if (id) {
        if (id == 101) {
...
function calculateInterest(principal, rate, time, bankRates = 1, bankName) {
    let interest = (principal * rate * time * bankRates) / 100;
    if (bankName) {
        console.log(bankName + " will charge total interest USD " + interest);
    }
    else {
        console.log("Total interest amount will be USD " + interest);
    }
    return interest;
}
calculateInterest(10000, 9, 3, 2, "Swiss Bank");
calculateInterest(10000, 9, 3);
const filterSmallWords = (...words) => {
    let smallWords = [];
    words.forEach((word) => {
        if (word.length <= 4) {
...
filterSmallWords("batter", "zoo", "care", "mate", "stale", "seek", "silence", "tail", "act", "many");
let mobiles = [
    {
        color: "Very Silver",
        price: 25000
